...
class mode:
    ...
    def setup_src(self, simulation, eps_background, matrix_format=DEFAULT_MATRIX_FORMAT):
        # compute the input power here using an only waveguide simulation
        self.compute_normalization(simulation, eps_background, matrix_format=matrix_format)

        # insert the mode into the waveguide
        self.insert_mode(simulation, simulation.src, matrix_format=matrix_format)

    def compute_normalization(self, simulation, eps_background, matrix_format=DEFAULT_MATRIX_FORMAT):
        ...
        norm_eps = np.ones((Nx, Ny)) * eps_background
        ...
        if self.direction_normal == "x":
            inds_y = original_eps[self.center[0], :] > eps_background
            norm_eps[:, inds_y] = eps_max
            new_center[0] = Nx - new_center[0]
        elif self.direction_normal == "y":
            inds_x = original_eps[:, self.center[1]] > eps_background
