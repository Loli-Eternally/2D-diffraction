...
class Simulation:
    ...
    def setup_modes(self, eps_background=1):
        # calculates
        for modei in self.modes:
            modei.setup_src(self, eps_background)
    ...
    def _check_inputs(self):
        ...
        assert len(self.NPML) == 2, "yrange must be a list of length 2, was supplied {}, which is of length {}".format(
            str(self.NPML), len(self.NPML))
    ...
    def init_design_region(self, design_region, eps_m, eps_background, style=''):
        ...
        if style == 'full':
            ...
            eps_halfway[design_region == 1] = (eps_m + eps_background) / 2
            ...
            eps_random = (eps_m - eps_background) * np.random.random(self.eps_r.shape) + eps_background
            ...
            eps_random = (eps_m - eps_background) * R_sym + eps_background
            eps_random[design_region == 0] = self.eps_r[design_region == 0]
            self.eps_r = eps_random

        self.rho = eps2rho(self.eps_r, eps_m, eps_background)
