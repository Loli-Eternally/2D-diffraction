...
"""
This is where you can custom define functions that create relative permittivity
given a set of geometric parameters.
See three_port, two_port, ortho_port, and accelerator for examples
"""
...
def three_port(L, H, w, d, l, spc, dl, NPML, eps_start, eps_background=1):
    ...
    wg_in = lambda x, y: (x < 0) * (
            np.abs(y - y_mid) < dl * wg_width_px / 2)  # note, this slight offset is to fix gridding issues
...
def two_port(L, H, w, l, spc, dl, NPML, eps_start, eps_background=1):
    ...
    eps_r = apply_regions([wg], xs, ys, eps_start=eps_start, eps_background=eps_background)
    design_region = apply_regions([box], xs, ys, 2) - 1

    return eps_r, design_region


def one_port(L, H, w, l, spc, dl, NPML, eps_start, eps_background=1, box_on=True):
    # CONSTRUCTS A ONE IN TWO OUT PORT DEVICE
    # L         : box length in L0
    # H         : box width  in L0
    # w         : waveguide widths in L0
...
def N_port(N, L, H, w, d, l, spc, dl, NPML, eps_start, eps_background=1):
    ...
    box = lambda x, y: (np.abs(x) < L / 2) * (np.abs(y - y_mid) < H / 2)
    wg_in = lambda x, y: (x < 0) * (
            np.abs(y - 0.01) < dl * wg_width_px / 2)  # note, this slight offset is to fix gridding issues
...
def N2N_port(N1, N2, L, H, w, d1, d2, l, spc, dl, NPML, eps_start, eps_background=1):
    # CONSTRUCTS A ONE IN N OUT PORT DEVICE
    # N1        : number of inputs
    # N2        : number of outputs
    # L         : box length in L0
    # H         : box width  in L0
    # w         : waveguide widths in L0
    # d         : distance between adjacent waveguide (centers) in L0
...
def ring_port(L, H, w, d, l, spc, dl, NPML, eps_start, eps_background=1):
    # CONSTRUCTS A ONE IN N OUT PORT DEVICE
    # R         : radius of ring
    # N2        : number of outputs
    # L         : box length in L0
    ...
    # define the regions
    box = lambda x, y: ((x ** 2 + y ** 2) < (d / 2 - 0.5 * w) ** 2) * ((x ** 2 + y ** 2) > (d / 2 - w * 1.5) ** 2)
    reg_list = [box]
    for i in range(2):
        y_i = (float(i) - float(2 - 1) / 2.0) * d
        wg_in = lambda x, y, y_i=y_i: (np.abs(y - y_i) < dl * wg_width_px / 2)
        reg_list.append(wg_in)
