...
def rhob2eps(rhob, eps_m, eps_background):
    # filtered density to permittivity
    return eps_background + rhob * (eps_m - eps_background)


def eps2rho(eps, eps_m, eps_background):
    # permittivity to density (ONLY USED FOR STARTING THE SIMULATION!)
    return (eps - eps_background) / (eps_m - eps_background)


def rho2eps(rho, eps_m, eps_background, W, eta=0.5, beta=100):
    rhot = rho2rhot(rho, W)
    rhob = rhot2rhob(rhot, eta=eta, beta=beta)
    eps = rhob2eps(rhob, eps_m=eps_m, eps_background=eps_background)
    return eps
...
def deps_drhob(rhob, eps_m, eps_background):
    return (eps_m - eps_background)
